...
<body>
	<%--
		navbar.jsp 페이지가 포함시키고 해당 페이지에 "current" 라는 파라미터 명으로 "member" 전달하기
		그러면 navbar.jsp 페이지에서는
		request.getParameter("current")를 사용해서 "member" 문자열을 읽어올 수 있다.
	 --%>
		...
		<table class="table table-striped">
			...
			<% for(MemberDto dto: list){%>
				<tr>
					...
					<td><%=dto.getName() %></td>
					<td><%=dto.getAddr() %></td>
					<td><a href="delete.jsp?num=<%=dto.getNum()%>">
